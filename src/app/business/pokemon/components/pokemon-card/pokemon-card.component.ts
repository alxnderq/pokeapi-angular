...
import { Pokemon, Types } from '../../models';
...
export class PokemonCardComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['pokemon'] && changes['pokemon'].currentValue) {
      const item: Pokemon = changes['pokemon'].currentValue
      item.types.map((types: Types): Types => {
        types.type.backgroundColor = `bg-${types.type.name}`
        return types;
      })
      this.pokemon = item;
